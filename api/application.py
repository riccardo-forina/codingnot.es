...
from date_converter import date_to_datetime

author =  {'name':'Riccardo Forina','email':'riccardo@forina.eu'}
utc = pytz.utc
...
def feed(articles): 
    ...
    fg.title('Coding notes by Riccardo Forina')
    fg.author(author)
    fg.link( href=request.url_root, rel='alternate' )
    fg.logo(make_external(url_for('static', filename="icons/favicon.ico")))
    fg.subtitle('This is a cool feed!')
    fg.link( href=request.url, rel='self' )
    fg.language('en')
    
    for article in articles:
        permalink = make_external(url_for('page', path=article.path))
        fe = fg.add_entry()
        fe.id(permalink)
        fe.title(article.meta['title'])
        fe.content( content=str(article.html), type='html' )
        fe.author(author)
        fe.link( href=permalink, rel='self' )
        fe.updated(utc.localize(date_to_datetime(article.meta.get('updated', article.meta['date']))))
        fe.published(utc.localize(date_to_datetime(article.meta['date'])))
